...
async def _fetch_account(session: aiohttp.ClientSession, account: dict, preset: str, since: str = "", until: str = "") -> dict:
    try:
    ...
    if since and until:
        ...
        insight_params["date_preset"] = preset

    insights_task = _get(session, f"{GRAPH_API}/{acc_id}/insights", insight_params)
...
async def fetch_all_accounts(preset: str, since: str = "", until: str = "") -> list:
    async with aiohttp.ClientSession() as session:
        tasks = [_fetch_account(session, acc, preset, since, until) for acc in ACCOUNTS]
...
async def fetch_single_account(idx: int, preset: str, since: str = "", until: str = "") -> dict:
    async with aiohttp.ClientSession() as session:
        return await _fetch_account(session, ACCOUNTS[idx], preset, since, until)
